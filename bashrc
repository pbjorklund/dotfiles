...
# Simple, reliable bash prompt that works well with VS Code and GitHub Copilot
# Shows: user@host:path$  (with colors if supported)
if [ "$color_prompt" = yes ]; then
    # Check if we're in a git repository and show branch if so
    __git_ps1_show_branch() {
        local branch
        if command -v git >/dev/null 2>&1; then
            branch=$(git branch --show-current 2>/dev/null)
            if [ -n "$branch" ]; then
                echo " \[\033[0;36m\]($branch)\[\033[0m\]"
            fi
        fi
    }
    
    # Set a clean, simple prompt
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]$(__git_ps1_show_branch)\$ '
else
    PS1='${debian_chroot:+($debian_chroot)}\u@\h:\w\$ '
fi
